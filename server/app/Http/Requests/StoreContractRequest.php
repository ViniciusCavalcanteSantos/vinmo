    ...
    public function rules(): array
    {
        ...
        if ($this->input('category') === 'graduation') {
            ...
            if($this->input('type') === 'university') {
                $conditionalRule = ['university_course' => ['required_if:type,university', 'nullable', 'string', 'max:120']];
            }
            if($this->input('type') === 'school') {
                $conditionalRule = ['school_grade_level' => ['required_if:type,school', 'nullable', Rule::in(['elementary_school', 'middle_school', 'high_school'])]];
            }
